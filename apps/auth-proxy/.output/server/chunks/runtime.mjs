...
function encodeQueryItem$1(key, value) {
  ...
  if (Array.isArray(value)) {
    return value
      .map((_value) => `${encodeQueryKey$1(key)}=${encodeQueryValue$1(_value)}`)
      .join("&");
  }
...
function stringifyQuery$1(query) {
  return Object.keys(query)
    ...
    .map((k) => encodeQueryItem$1(k, query[k]))
    .filter(Boolean)
    .join("&");
}
...
function hasProtocol$1(inputString, opts = {}) {
  ...
  return (
    PROTOCOL_REGEX$1.test(inputString) ||
    (opts.acceptRelative ? PROTOCOL_RELATIVE_REGEX$1.test(inputString) : false)
  );
}
...
function stringifyParsedURL$1(parsed) {
  ...
  const proto =
    parsed.protocol || parsed[protocolRelative$1]
      ? (parsed.protocol || "") + "//"
      : "";
...
storage.mount(
  ...
  unstorage_47drivers_47fs_45lite({
    driver: "fsLite",
    base: "/Users/Palmford/Desktop/cooper/apps/auth-proxy/.data/kv",
  }),
);
